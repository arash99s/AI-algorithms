...
class Node:
    def __init__(self , name , heuristic, traveled , previousNode) -> None:
        self.name = name
        self.heuristic = heuristic
        self.traveled = traveled
        self.previousNode = previousNode
    def get_total_distance(self):
        return self.heuristic + self.traveled
...
def appendOrUpdate(openNodes:list , neighborNode:Node):
    for index , node in enumerate(openNodes):
        if neighborNode.name == node.name: 
            if neighborNode.traveled < node.traveled:
                openNodes[index] = neighborNode
            return
    openNodes.append(neighborNode) # not exist and must be update

openNodes = []
currentNode = Node('A' , heuristics['A'] , 0, None)
while True:
    neighbors = find_neighbors(currentNode.name)
    for node in neighbors:
        traveled = currentNode.traveled + find_edge_distance (currentNode.name , node)
        neighborNode = Node(node , heuristics[node], traveled, currentNode)
        appendOrUpdate(openNodes , neighborNode)
...
print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
print('length:' , currentNode.get_total_distance())
path = []
